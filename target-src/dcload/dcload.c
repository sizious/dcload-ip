...
#define ONSCREEN_REFRESH_SCALED                                                                    \
    ((unsigned long long int)ONSCREEN_DHCP_LEASE_TIME_REFRESH_INTERVAL *                           \
     (unsigned long long int)PERFCOUNTER_SCALE)

// CPU clock cycles need doubles in 1 count = 1 cycle mode
#ifndef BUS_RATIO_COUNTER
__attribute__((aligned(8))) static const unsigned int const_perf_freq[2] = {
    PERFCOUNTER_SCALE_DOUBLE_HIGH, PERFCOUNTER_SCALE_DOUBLE_LOW};
__attribute__((aligned(8))) static const unsigned int const_32[2] = {
    0x41f00000, 0x00000000}; // 2^32 in SH4 double format
...
static const char *waiting_string =
    "Waiting for IP..."; // Waiting for IP indicator. 15 visible characters to match IP address
                         // string's visible 15 characters
...
static char dhcp_lease_time_string[11] = {
    0}; // For converting lease time to seconds. 10 characters + null term. Max lease is
        // theoretically 4294967295, but really is 1410902 due to perf counters.
...
static void set_ip_from_file(void) {
    ...
    // IP is currently stored, for example, ip[0] 192-168-0-11 ip[3] (this is big endian data on a
    // little endian system), which is what networking packets expect
    our_ip = ntohl(our_ip);
    // Now it's stored ip[0] 11-0-168-192 ip[3] (this is little endian data on a little endian
    // system), which make_ip handles when building packets.
}
...
static void update_lease_time_display(unsigned int new_time) {
    ...
    clear_lines(424, 48,
                global_bg_color); // Clear 48 lines so DHCP retry attempts text is also cleared
...
void set_ip_dhcp(void) {
    ...
    unsigned long long int long_dhcp_lease_time =
        (unsigned long long int)dhcp_lease_time * (unsigned long long int)(PERFCOUNTER_SCALE);
    ...
    unsigned long long int *old_dhcp_lease_updater =
        (unsigned long long int *)old_dhcp_lease_updater_array;

    // Check if lease is still active, renewal threshold is at 50% lease time. '>> 1' is '/2'.
    // NOTE: GCC apparently can't handle the concept of dividing 64-bit numbers on SH4, even by a
    // power of two. It adds over 1kB of extra code at the mere sight of it, weirdly, so we do the
    // shift manually here.
    if(__builtin_expect(dhcp_lease_time && (!dont_renew) &&
                            ((long_dhcp_lease_time >> 1) < (*current_counter)),
                        0)) {
        dhcp_lease_time = 0; // This disables DHCP renewal unless it gets updated with a valid
                             // value. Its dual-purpose is a renewal code enabler.
        ...
        int renew_result =
            dhcp_renew((unsigned int *)&our_ip); // So that GCC doesn't warn about volatile

        if(renew_result == -2) {
            // Not an error, it just means IP was no longer valid. Time to get a new one!
            // We're supposed to wait until 87.5% of the lease time has elapsed to do a new
            // discover.
            dont_renew = 1;
        }
    ...
    // supposed to be stored in an unsigned long long int (64-bit, in SH4's case).
    else if((long_dhcp_lease_time >= (*current_counter)) &&
            ((*current_counter) > ((*old_dhcp_lease_updater) + ONSCREEN_REFRESH_SCALED - 1ULL))) {
        ...
        // Need to somehow divide the (long_dhcp_lease_time - current_counter) difference, which is
        // 64-bit, without 64-bit divide...
        ...
        undefined_tmp_fpscr =
            old_fpscr |
            (3 << 19); // Set FPSCR.PR to double-precision mode & FPSCR.SZ to pair-move mode
        ...
        asm volatile(
            ...
            ".skip_sign:\n\t"  // What's nice about doubles is that no information is lost in 32-bit
                               // sign conversion
            "lds %[in_dbl1],FPUL\n\t" // Load high half
            ...
            : [in_dbl0] "r"(((unsigned int *)&difference)[0]), [in_dbl1] "r"(
        ...
        // Bad form on the program's part if so, but I don't want to be responsible for breaking
        // things.
...
//        unsigned int remaining_lease = (unsigned int)((difference * 43ULL) >> 33); // <-- Assumes
//perfcounter values are supposed to be divided by 200,000,000 (well, 99750000 x 2, really)
#else
...
#if(DCLOAD_PMCR == 1)
        uint_to_string(*((volatile unsigned short *)PMCR1_CTRL_REG),
                       (unsigned char *)dhcp_lease_time_string);
#elif(DCLOAD_PMCR == 2)
        uint_to_string(*((volatile unsigned short *)PMCR2_CTRL_REG),
                       (unsigned char *)dhcp_lease_time_string);
#else
    ...
    unsigned long long int eighty_seven_point_five =
        (long_dhcp_lease_time >> 1) + (long_dhcp_lease_time >> 2) +
        (long_dhcp_lease_time >> 3); // 0.5 + 0.25 + 0.125 = 0.875, or 87.5%
    ...
    // Also check if renew was NAK'd, and if it was, are we at the 87.5% threshold for a new
    // discover?
    if(__builtin_expect(((our_ip & 0xff000000) == 0) ||
                            (dont_renew && (eighty_seven_point_five < (*current_counter))),
                        0)) {
        dont_renew = 0;
        dhcp_waiting_mode_display();
...
int main(void) {
    ...
    */
    ...
    // (Don't worry, this won't run again if it's already enabled; that would accidentally reset the
    // lease timer!)
#ifndef BUS_RATIO_COUNTER
