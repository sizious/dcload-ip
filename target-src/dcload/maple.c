...
// Make GCC 32-byte align it in the .data section since GCC aligns the binary relative to
// 0x8c010000.
__attribute__((
    aligned(32))) volatile unsigned char dmabuffer[MAPLE_DMA_SIZE]; // Here's a global array
...
void *maple_docmd(int port, int unit, int cmd, int datalen, void *data) {
    ...
    *sendbuf++ =
        datalen | (port << 16) | 0x80000000; // NOTE: These 3 writes use the uncacheable area
    ...
    *sendbuf++ = (cmd & 0xff) | (to << 8) | (from << 16) | (datalen << 24);
    ...
    if(datalen > 0) {
        //    memcpy(sendbuf, data, datalen << 2); // sendbuf is 32-byte aligned, offset by 12. data
        //    is 8-byte aligned, offset by 4 due to port, unit, cmd, & datalen
        // So memcpy_32bit the first 4 bytes to make it all 8-byte aligned (remaining sendbuf will
        // be 16-byte aligned and remaining data will be 8-byte aligned)
        memcpy_32bit(sendbuf, data, 4 / 4);
        SH4_aligned_memcpy((void *)(((unsigned int)sendbuf + 4) & 0x1fffffff),
                           (void *)(((unsigned int)data + 4) & 0x1fffffff),
                           datalen - 1); // use copy-back memory area for speed boost
        CacheBlockWriteBack((unsigned char *)((unsigned int)sendbuf & 0x1fffffe0),
                            ((datalen * 4) + 31) /
                                32); // Synchronize memory with opcache in 32-byte blocks, sendbuf
                                     // is already 32-byte aligned
