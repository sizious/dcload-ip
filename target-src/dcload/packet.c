...
// The two checksums here are different because the UDP one needs a "pseudo-header," while the IP
// one doesn't
unsigned short checksum(unsigned short *buf, int count, int is_odd) {
    ...
    if(is_odd) {
        sum += (unsigned short)(*(
            (unsigned char *)
                buf)); // The sum is a little-endian sum, so an odd byte will be an 8-bit int
...
unsigned short checksum_udp(unsigned short *buf_pseudo, unsigned short *buf_data, int datacount,
    ...
    if(is_odd) {
        sum += (unsigned short)(*(
            (unsigned char *)
                buf_data)); // The sum is a little-endian sum, so an odd byte will be an 8-bit int
...
__attribute__((aligned(4))) unsigned char
    pseudo_array[PSEUDO_H_LEN]; // Here's a global array (not really global, but... search terms)

// UDP packet length should always be an even number. It's the length of the UDP payload data
// specified by the 'data' variable.
void make_udp(unsigned short dest, unsigned short src, int length, ip_header_t *ip,
              udp_header_t *udp) {
    ip_udp_pseudo_header_t *pseudo =
        (ip_udp_pseudo_header_t *)((unsigned int)pseudo_array & 0x1fffffff);
    ...
    udp->checksum =
        checksum_udp((unsigned short *)pseudo, (unsigned short *)udp->data, length / 2, length % 2);
    if(udp->checksum == 0)
        udp->checksum = 0xffff;
}
