...
// Ethernet header (14) + ip header (20) + udp header (8) + command struct (12) = 54 bytes before
// command->data So to align the actual command->data to 8 bytes, need to align the array to 8 bytes
// and offset the start by 2 bytes. 1516 - 2 = 1514, and 54 bytes later (56 bytes from the start of
// the array) is aligned to 8 bytes since 56 = 8 * 7. Also: ip header is aligned to 8 bytes,
// udp/icmp header is aligned to 4 bytes, and command struct (or other payload header) is aligned to
// 4 bytes Use 1518 because the max data copied over G2 is 1516 by pkt_to_mem, then with the 2-byte
// shift is 1518. But SH4_mem_to_pkt() reads 4 bytes beyond the end of a size, so (1514 + 3)/4
// becomes 1516/4, which will read 1520 bytes. So we end up with 1520 to account for that. Plus 1520
// is the nearest multiple of 8 greater than 1514, too. And then you throw caching in, with does
// things in 32-byte blocks, so we need to extend to the nearest multiple of 32 >1514, or 1536,
...
static char uint_string_array[9] = {0};
    ...
    PMCR_Read(DCLOAD_PMCR, first_array);
        ...
        unsigned int loop_difference = (unsigned int)(*(unsigned long long int*)second_array -
*(unsigned long long int*)first_array);
