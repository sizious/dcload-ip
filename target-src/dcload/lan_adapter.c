...
adapter_t adapter_la = {"LAN Adapter (HIT-0300)",
                        {0}, // Mac address
...
int la_bb_init(void) {
    ...
    REGW(15) = 0;    // Writing 0 to this one does nothing (using full duplex, now, so SQE doesn't
                     // matter. Polarity is auto-corrected and LINK FAIL is checked by la_bb_loop())
...
void la_bb_stop(void) {
    ...
    */
    while(
        !(REG(0) & 0x80)) { // wait for any prior transmit of all packets in transmit buffer to finish
        net_sleep_ms(2);
    }
    // We could reset first_transmit, or we could leave the TMT OK bit high for the next time
    // (better)
...
// Tx time end
    ...
    // This will be blocked on the first transmit because setting ENA DLC (REG(6) bit 0x80) clears
    // TMT OK... So keep track of whether this is the first transmit or not
    if(__builtin_expect(
           first_transmit,
           0)) // if(first_transmit), but telling GCC we usually expect first_transmit to be 0
    {
        first_transmit = 0;
    }
    else {
        while(!(REG(0) & 0x80)) // wait for prior transmit (in the other bank, since dcload uses
                                // dual-bank mode) to finish
        {
            net_sleep_ms(2);
        }
...
void la_bb_loop(int is_main_loop) {
    ...
    while(!escape_loop) {
        ...
        // 10Base-T heartbeat signal is this chip's LINK FAIL mechanism, not the "carrier sense"
        // stuff
        ...
        unsigned int reg_aggregate = ((unsigned int)(REG(15)) << 24) |
                                     ((unsigned int)(REG(4)) << 16) |
