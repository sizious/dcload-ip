...
void PMCR_Init(unsigned char which, unsigned char mode,
               unsigned char count_type) // Will do nothing if perfcounter is already running!
{
...
// Enable "undocumented" performance counters (well, they were undocumented at one point. They're
// documented now!)
void PMCR_Enable(unsigned char which, unsigned char mode, unsigned char count_type,
                 unsigned char reset_count) // Will do nothing if perfcounter is already running!
{
    ...
    if(which == 1) {
        ...
        asm volatile(
            ...
            : [outh] "=m"(out_array[1]), [outl] "=m"(out_array[0]), [reg1h] "+&r"(pmcr1_regh),
              [reg1l] "+r"(pmcr1_regl) // SH4 can't mov an immediate longword into a register...
            :                          // no inputs
            :                          // no clobbers
        );
    }
        ...
        asm volatile(
            ...
            : [outh] "=m"(out_array[1]), [outl] "=m"(out_array[0]), [reg2h] "+&r"(pmcr2_regh),
              [reg2l] "+r"(pmcr2_regl) // SH4 can't mov an immediate longword into a register...
            :                          // no inputs
            :                          // no clobbers
        );
    }
...
unsigned long long int PMCR_RegRead(unsigned char which) {
    ...
    if(which == 1) {
        ...
        // counter 1
        // output value = (unsigned long long int)(*((volatile unsigned int*)PMCTR1H_REG) & 0xffff)
        // << 32 | (unsigned long long int)(*((volatile unsigned int*)PMCTR1L_REG));
        asm volatile(
            "mov.l @%[reg1h],%[reg1h]\n\t" // read counter (high)
            "mov.l @%[reg1l],%[reg1l]\n\t" // read counter (low)
            "extu.w %[reg1h],%[reg1h]\n"   // zero-extend high, aka high & 0xffff
        ...
        // counter 2
        // output value = (unsigned long long int)(*((volatile unsigned int*)PMCTR2H_REG) & 0xffff)
        // << 32 | (unsigned long long int)(*((volatile unsigned int*)PMCTR2L_REG));
        asm volatile(
            "mov.l @%[reg2h],%[reg2h]\n\t" // read counter (high)
            "mov.l @%[reg2l],%[reg2l]\n\t" // read counter (low)
            "extu.w %[reg2h],%[reg2h]\n"   // zero-extend high, aka high & 0xffff
