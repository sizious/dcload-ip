...
// Tx time
    ...
    unsigned char *copyback_pkt_base =
        (unsigned char *)((unsigned int)(&(pkt[-2])) &
                          0x1fffffff); // copyback base in cached memory area
...
static void pktcpy(unsigned char *dest, unsigned char *src,
    ...
    // n now
    memcpy_32bit(dest, (unsigned char *)0x01848000,
                 (n + 2 + 3) / 4); // Lol this is as fast as the asm functions
    CacheBlockInvalidate((unsigned char *)0x01848000,
...
// Rx time
            ...
            // pktcpy(raw_current_pkt, pkt, pkt_size); // SH4_pkt_to_mem() will shift it by 2 for
            // current_pkt
            pktcpy((unsigned char *)((unsigned int)raw_current_pkt & 0x1fffffe0), pkt,
                   pkt_size); // SH4_pkt_to_mem() will shift it by 2 for current_pkt
