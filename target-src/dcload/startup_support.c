...
void STARTUP_Init_Video(unsigned char fbuffer_color_mode) {
    ...
    *(volatile uint32_t *)0xff80002c =
        ((*(volatile uint32_t *)0xff80002c) & 0xfff0ffff) | 0x000a0000;
    ...
    *(volatile uint32_t *)0xa0702c00 =
        ((*(volatile uint32_t *)0xa0702c00) & 0xfffffcff) | cable_mode;
...
void STARTUP_Set_Video(unsigned char fbuffer_color_mode) {
    ...
    uint32_t bpp_mode_size = fbuffer_color_mode + 1 +
                             (0x1 ^ ((fbuffer_color_mode & 0x1) |
                                     (fbuffer_color_mode >> 1))); // Add another 1 only if 0b00
    ...
    if(!cable_mode) // VGA 640x480 @ 60Hz
    {
        ...
        *(volatile uint32_t *)0xa05f804c =
            (horiz_active_area * bpp_mode_size) / 8;   // for PVR to know active area width
        *(volatile uint32_t *)0xa05f8040 = 0x00000000; // Border color in RGB0888 format
        *(volatile uint32_t *)0xa05f805c =
            (1 << 20) | ((vert_active_area - 1) << 10) |
            (((horiz_active_area * bpp_mode_size) / 4) -
             1); // progressive scan has a 1 since no lines are skipped
        ...
        *(volatile uint32_t *)0xa05f80e0 = 0x03f1933f;
        ...
        uint32_t scan_area_size_bytes =
            scan_area_size * bpp_mode_size; // This will always be divisible by 4

        // Reset framebuffer address
        *(volatile uint32_t *)0xa05f8050 =
            0x00000000; // BootROM sets this to 0x00200000 (framebuffer base is 0xa5000000 + this)
        *(volatile uint32_t *)0xa05f8054 =
            0x00000000; // Same for progressive, resetting the offset gets us 2MB VRAM back after
                        // BootROM is done with it
        ...
        *(volatile uint32_t *)0xa05f804c =
            (horiz_active_area * bpp_mode_size) / 8;   // for PVR to know active area width
        *(volatile uint32_t *)0xa05f8040 = 0x00000000; // Border color in RGB0888 format
        *(volatile uint32_t *)0xa05f805c = ((((horiz_active_area * bpp_mode_size) / 4) + 1) << 20) |
                                           (((vert_active_area / 2) - 1) << 10) |
                                           (((horiz_active_area * bpp_mode_size) / 4) - 1);
        ...
        *(volatile uint32_t *)0xa05f8050 =
            0x00000000; // BootROM sets this to 0x00200000 (framebuffer base is 0xa5000000 + this)
        *(volatile uint32_t *)0xa05f8054 =
            horiz_active_area * bpp_mode_size; // This is for interlaced, resetting the offset gets
                                               // us 2MB VRAM back after BootROM is done with it
        ...
        *(volatile uint32_t *)0xa05f8044 |= 1;
        ...
        // Interlaced PAL can actually have 528 active vertical lines on the Dreamcast (48 more than
        // NTSC!) In theory a developer could hide a secret message for PAL players in this area
        vert_active_area += 48;
#endif
