...
__attribute__((aligned(32))) unsigned char
    raw_pkt_buf[RAW_TX_PKT_BUF_SIZE]; // Here's a global array. Global packet transmit buffer.
// Need to offset the packet by 2 for the command->data after headers to always be aligned to 8
// bytes The performance gains are well worth the 2 wasted bytes.
...
static void process_icmp(ether_header_t *ether, ip_header_t *ip, icmp_header_t *icmp) {
    if(icmp->type == 8) /* echo request */
    {
        ...
        icmp->checksum = 0;
        icmp->checksum =
            checksum((unsigned short *)icmp, ip_length / 2 - 2 * ip_ihl, ip_length % 2);
        if(i != icmp->checksum)
            return;
...
static void process_udp(ether_header_t *ether, ip_header_t *ip, udp_header_t *udp) {
    ...
    if(udp->checksum != 0)
        i = checksum_udp((unsigned short *)pseudo, (unsigned short *)udp->data, udp_data_length / 2,
                         udp_data_length %
                             2); // integer divide; need to round up to next even number
    else
        i = 0;
    ...
    if(__builtin_expect(udp_pkt_data->op == DHCP_OP_BOOTREPLY, 0)) // DHCP ACK or DHCP OFFER
    {
        if(!handle_dhcp_reply(ether->src, udp_pkt_data,
                              udp_data_length)) // -8 because udp->length includes 8-byte udp header
        {                                       // -1 is true in C
            // If we got a DHCP packet that belongs to some other machine, e.g. some machine
            // requires a broadcasted address instead of a unicasted one, don't escape the loop
            // since we didn't get the packet we needed.
            escape_loop = 1;
        }
    }
    else {
        ...
        // Only one of these will ever match at a time. What we can do is set this variable to 0
        // after compare succeeds. There is no id of 0, as they're all 4-character,
        // non-null-terminated strings. Unfortunately packet headers are 42 bytes, which is NOT a
        // multiple of 4. It is a multiple of 2, though, so we can do this without crashing:
        //        __attribute__((aligned(4))) unsigned int pkt_match_id = ((unsigned int) *(unsigned
        //short*)&command->id[2] << 16) | (unsigned int) *(unsigned short*)command->id;
        ...
        // This one is the most likely to be called the most often, so put it first and tell GCC
        // it's likely to be called
        if(__builtin_expect((pkt_match_id) && (!memcmp_32bit_eq(&pkt_match_id, CMD_PARTBIN, 4 / 4)),
                            1)) {
        ...
        // Make ethernet header in transmit packet buffer since all below functions have a response
        // packet (except reboot)
...
static void process_mine(unsigned char *pkt) {
    ...
    ip_header->checksum =
        checksum((unsigned short *)ip_header, 2 * ip_ihl, 0); // 2*ip_ihl because unsigned shorts
...
void process_pkt(unsigned char *pkt) {
    ...
    // Destination ethernet header is the first thing in the packet, so it's always aligned to 2
    // bytes
